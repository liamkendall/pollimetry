...
author: Liam Kendall, Romina Rader, Vesna Gagic, Daniel Cariveau, Matthias Albrecht,
  Katherine Baldock, Breno Freitus, Juanita Rodriguez, Laura Russo, Louis Suter and
  Ignasi Bartomeus
date: "18/05/2018"
output:
...
hov.full=lmer(log(Spec.wgt) ~ log(IT) + Region+ Sex + Subfamily+ #fix
            ...
            log(IT):Subfamily + log(IT):Region+ (1|Measurement/Species)
...
hov.red=lmer(log(Spec.wgt) ~ log(IT) + Subfamily + #interactions
            log(IT):Subfamily + (1|Measurement/Species)
            ,REML=FALSE,data=hov_mean)
...
bee_1=lmer(formula=log(Spec.wgt) ~ log(IT)  + Family + Sex  + Region+#fix factors
             log(IT):Family  + log(IT):Sex + log(IT):Region+ #interactions
             (1|Measurement/Species),data=bee_mean)
bee_2=lmer(formula=log(Spec.wgt) ~ log(IT)  + Family + Region  + Sex+ #fix factors
            log(IT):Family  + log(IT):Region + #interactions
            (1|Measurement/Species),data=bee_mean)
bee_3=lmer(formula=log(Spec.wgt) ~ log(IT)  + Family + Region  + #fix factors
             log(IT):Family  + log(IT):Region + #interactions
             (1|Measurement/Species),data=bee_mean)
bee_4=lmer(formula=log(Spec.wgt) ~ log(IT)  + Family + Sex + #fix factors
             log(IT):Family + log(IT):Sex + #interactions
             (1|Measurement/Species),data=bee_mean)
bee_5=lmer(formula=log(Spec.wgt) ~ log(IT)  + Family  + #fix factors
             log(IT):Family + #interactions
             (1|Measurement/Species),data=bee_mean)
bee_6=lmer(formula=log(Spec.wgt) ~ log(IT) + #interactions
             (1|Measurement/Species),data=bee_mean)
...
```{r, results='asis'}
stargazer(bee_1,bee_2, bee_3, bee_4, bee_5, bee_6, header=FALSE,type='latex')

```
#Hoverflies
```{r}
hov_1=lmer(formula=log(Spec.wgt) ~ log(IT) + Sex +#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_2=lmer(formula=log(Spec.wgt) ~ log(IT) + Sex+ Region+#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_3=lmer(formula=log(Spec.wgt) ~ log(IT)+Sex + Region+Subfamily+#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_4=lmer(formula=log(Spec.wgt) ~ log(IT) +Sex + Subfamily+#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_5=lmer(formula=log(Spec.wgt) ~ log(IT) * Sex +#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_6=lmer(formula=log(Spec.wgt) ~ log(IT)*Sex + Region+#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_7=lmer(formula=log(Spec.wgt) ~ log(IT)  + Subfamily+#Fixed
             (1|Measurement/Species),data=hov_mean)
hov_8=lmer(formula=log(Spec.wgt) ~ log(IT)+#Fixed
             (1|Measurement/Species),data=hov_mean)
...
model1<-bee_mean%>%mutate(Fold=rep(0,nrow(bee_mean)),holdoutpred=rep(0,nrow(bee_mean)),MSE=rep(0,nrow(.)),RMSE=rep(0,nrow(.)),MAE=rep(0,nrow(.)),R2=rep(0,nrow(.)),AIC=rep(0,nrow(.)),BIC=rep(0,nrow(.)))
  for(i in 1:5){
    ...
    newlm=lme4::lmer(formula=log(Spec.wgt) ~ log(IT)  + Family + Sex  + Region+#fix factors
                       log(IT):Family  + log(IT):Sex + log(IT):Region+ #interactions
                       (1|Measurement/Species),data=train)
...
#Box plots
bee_RMSE=RMSE_sets%>%gather(.,c("LM1","PG1","JC89"),key ="Model",value = "RMSE")%>%ggplot(aes(x=Model,y=RMSE,fill=Model))+geom_boxplot()+theme_bw()+ theme(aspect.ratio=1,axis.title.x=element_blank(),
...
#Comparing RMSE among models (differences)
```{r}
exp(mean(RMSE_sets$PG2)-mean(RMSE_sets$PG1))

mean(RMSE_sets$LM1)-mean(RMSE_sets$PG1)
mean(RMSE_sets$LM2)-mean(RMSE_sets$PG1)
```
...
pres.lme=lmer(log(Spec.wgt)~log(IT)+Pres.time+Sex+(1|Country/Species),bee_pres)
