...
Dan's data is in - he has more to add in a couple of weeks

Is the syntax of preservative time model right/appropriate?
...
###PREDICTIVE FRAMEWORKS UNCOMMON IN ECOLOGY

TRAINING / TEST SET ETC ETC
...
We used the genus-level phylogeny provided within Hedtke et al. (2013)...
...
options(na.action = "na.fail")
bee_fe_lme=lmer(log(Spec.wgt) ~ log(IT)  + Family + Region + #fix factors
                              log(IT):Family + log(IT):Region + #interactions
                              (1|Measurement)+(1|Species),REML=FALSE,bee_mean)

bee_dre_lme=dredge(bee_fe_lme,beta="none",rank="AIC") #think about "sd" and "AICc". AIC show same pattern.
head(bee_dre_lme)
summary(bee_fe_lme)
```
...
WGT<-as.matrix((WGT))[,1]
obj<-contMap((bee_pruned),WGT,fsize= 0.4,cex=1)
...
Bootstrap isn't available within a package - Can probably do it with a loop but im not experienced enough to figure out the coding.
...
I repeated this process with the full bee-data set rather than the means.
...
plot((IT)~(Spec.wgt),bee_all_test,ylab="ITD (mm)",xlab="Dry weight (mg)")
...
##SUPP## Intraspecific variation
We assessed intraspecific variation in trait mesaurements. We plotted mean against increasing sample size to estimate the adequate sample size whereby mean variance stabilised. ANY IDEAS HOW TO TEST THIS?

For five most speciose species: Homalictus urbanus (n = 251), Lasioglossum pauxillum (n = 113), Bombus lucorum (n = 111), Andrena flavipes (n = 75) and Lasioglossum lanarium (n = 68), the sample size required for the mean to fit within the confidence intervals of the final mean. 
