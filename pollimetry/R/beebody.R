...
#' @param Climate Climate where specimens collected. Accepted countries are... 
...
#' @examples
#' beebody(c(10,5,2))
#' beebody(IT=c(10,5,2), Climate = rep("bwh",3), Family = rep("Apidae", 3), Latitude = rep(30, 3), Sex = rep("Male",3)) 
#' 
#' @references 
#' @export

beebody <- function(IT, Climate = NULL, Family = NULL, Latitude = NULL, Sex = NULL, data = bee_train){
  if(data = bee_train){
    load(bee_train)
  } else{
    load(bee_all)
  } #Think more about this, and how much flexibility we give.
  if(any(!Climate %in% c("Australia", #need to allow NULL
                  "Spain",
                  "Britain",
                  "Ireland", 
                  "Germany",
                  "USA"))) { #THIS limits to actual countries, make more generic? Regions?
    stop("Climate should be one of 'Australia', 'Spain', 'Britain', 'Ireland', 'Germany' or 'USA'")
  }
  ...
  if(any(!Sex %in% c("Male","Female"))) {
    ...
    #Think about the BEST model. Latitude? Climate may be overfitting the data? Sampling bias??
    Model_parameters <- tidy(summary(lm(formula = log(Spec.wgt) ~ 0 + Climate +
                                        Family + log(IT) + Latitude + Sex + 
                                        Climate:log(IT), data = data))) #Not necessarily this data set
    
    Model_parameters=bee_models$R1
    Model_parameters[,1] = c("Intercept","bwh","cfa", "cfb", 
            "csa","Apidae","Colletidae","Halictidae","Megachilidae","Melittidae","Latitude","IT",
                "Male","bwh","cfa","cfb","csa","Latitude")
     
     Climate_intercepts=Model_parameters[1:5,]
     colnames(Climate_intercepts)[1] <- "Climate" 
     
     Family_intercepts=Model_parameters[6:10,]
     colnames(Family_intercepts)[1] <- "Family"
     Family_intercepts[6,]=c("Andrenidae",as.numeric(0),NA,NA,NA)
     Family_intercepts[,2] <- as.numeric(Family_intercepts[,2]) 
     
     Sex_intercept=Model_parameters[13,]
     colnames(Sex_intercept)[1] <- "Sex"
     Sex_intercept[2,]=c("Female",0,0,0,0)
     Sex_intercept[,2] <- as.numeric(Sex_intercept[,2]) 
     
     Lat_int=Model_parameters[18,]
     colnames(Climate_int)[1] <- "Latitude"
     
     Climate_intercepts2=merge(data.frame(id = c(1:length(Climate)), Climate = Climate), 
           Climate_intercepts, all.x = TRUE) #MAKE SURE DO NOT BERAKS WITH NULL
     Climate_intercepts2 <- Climate_intercepts2[order(Climate_intercepts2$id),]
     ...
     Clim_int_intercepts2=merge(data.frame(id = c(1:length(Climate)), 
                                         Climate = Climate), 
                                Climate_int)
     Clim_int_intercepts2 <- Clim_int_intercepts2[order(Clim_int_intercepts2$id),]
      #Up to Liam :)
     out = exp(Climate_intercepts2$estimate++
                 ...
                 log(IT)*Clim_int_intercepts2$estimate+log(IT)*Lat_int$estimate)
     out
  }
}
...
#if(is.null(Climate)){
#  Climate_intercepts2$estimate <- #make it the mean response? and Co_int_intercepts2
#}
#if(is.null(Family)){
  #idem...
#}
