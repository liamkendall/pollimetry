...
#Bees
BEE1=ggplot(bee_all,aes(x=log(Spec.wgt),y=log(IT),colour=Region))+coord_equal(xlim=c(-2,6),ylim=c(-1,2))+
geom_point(aes(),lwd=2,shape=1)+theme_bw()+theme(aspect.ratio=1)+ggtitle("ITD")
BEE1
BEE2=ggplot(bee_all,aes(x=log(Spec.wgt),y=log(IT),colour=Family))+coord_equal(xlim=c(-2,6),ylim=c(-1,2))+
  geom_smooth(method="lm", formula=y~x,se=F)+
  geom_point(aes(),lwd=2,shape=1)+theme_bw()+theme(aspect.ratio=1)+ggtitle("ITD")
BEE2
BEE3=ggplot(bee_all,aes(x=log(Spec.wgt),y=log(IT),colour=Sex))+coord_equal(xlim=c(-2,6),ylim=c(-1,2))+
  geom_smooth(method="lm", formula=y~x,se=F)+
